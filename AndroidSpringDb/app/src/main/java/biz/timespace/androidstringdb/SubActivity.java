...
public class SubActivity extends ActionBarActivity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String url = "http://192.168.1.16:8080/android/list"; // 반드시 유효한 IP를 사용해야 합니다. localhost.127.0.0.1불가
        PostResponseAsyncTask readTask = new PostResponseAsyncTask(SubActivity.this, false, new AsyncResponse() {//, postData없이 GET 파라미터로 사용 예
            ...
            public void processFinish(String result) {
                ...
                ArrayList<MemberVO> memberVOList = //Json result 값을 Member클래스에 매핑하면서 memberList에 입력
                        new JsonConverter<MemberVO>().toArrayList(result, MemberVO.class);

                names = new ArrayList<String>();
                for(MemberVO value: memberVOList){ //memberList에서 개별 값 매핑하면서 names에 입력
                ...
                for(MemberVO m: memberVOList){ //memberList에서 개별 값 매핑하면서 names에 입력
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        DialogInterface.OnClickListener deleteListener = new DialogInterface.OnClickListener(){
            ...
            public void onClick(DialogInterface arg0, int arg1){
                //DB삭제
                String url = "http://192.168.1.16:8080/android/delete/"+data; //스프링(자바)한경일때
